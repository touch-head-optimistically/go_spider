...
func (this *Spider) AddUrlWithHeader(url string, respType string, header map[string]string) *Spider {
	customHeader := make(http.Header)
	for headerKey, headerVal := range header {
		customHeader.Set(headerKey, headerVal)
	}
	req := request.NewRequest(url, respType, "", "GET", "", customHeader, nil, nil, nil)
	this.AddRequest(req)
	return this
}
